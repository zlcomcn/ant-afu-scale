...
⚠️ 原始APK的体脂计算算法被阿里InstantRun热补丁机制挖空，
反编译得到的 `ICAlgorithmManager.calc()` 是空壳。
...
反编译发现原始APK中的 `ICAlgorithmManager.calc()` 被阿里InstantRun热补丁机制
在编译时剥离，运行时从阿里服务器动态加载。因此二进制 APK 中不包含算法本体。
